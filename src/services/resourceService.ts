...
export const resourceService = {
    ...
    addPlaylist: async (userId: string, _playlistUrl: string, playlistId: string) => {
        ...
        if (!playlistInfo) throw new Error('Failed to fetch playlist information');
    ...
    getUserResources: async (userId: string, statusFilter?: ResourceStatus) => {
        ...
        if (statusFilter) resources = resources.filter(r => r.status === statusFilter);
        return resources;
    },

    // Get all resources (unfiltered, for aggregation)
    getResources: async (userId: string): Promise<Resource[]> => {
        const resourcesRef = collection(db, USERS_COLLECTION, userId, 'resources');
        const q = query(resourcesRef, orderBy('dateAdded', 'desc'));
        const snapshot = await getDocs(q);
    ...
    getResource: async (userId: string, resourceId: string) => {
        ...
        if (snapshot.exists()) return { id: snapshot.id, ...snapshot.data() } as Resource;
        return null;
    },
    ...
    updateProgress: async (userId: string, resourceId: string, playedSeconds: number, totalSeconds: number) => {
        ...
        if (totalSeconds > 0 && playedSeconds >= totalSeconds * 0.95) status = 'watched';
    ...
    // Update generic resource fields
    updateResource: async (userId: string, resourceId: string, updates: Partial<Resource>): Promise<void> => {
        const resourceRef = doc(db, USERS_COLLECTION, userId, 'resources', resourceId);
        await updateDoc(resourceRef, updates as any);
    },
